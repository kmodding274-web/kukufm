...
// Body parsers (JSON, Form-Urlencoded aur Raw Text teeno ke liye)
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
app.use(express.text({ type: '*/*' }));

// Hardcoded Premium Request Headers
...
// Hardcoded Token Refresh Payload Body
...
app.all('*', async (req, res) => {
    ...
    try {
        ...
        // Agar POST request mein refresh token bheja ja raha hai, toh payload ko hardcoded value se replace kar do
        if (req.method === 'POST' && (req.url.includes('/token') || JSON.stringify(req.body).includes('refresh_token'))) {
